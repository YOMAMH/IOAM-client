...
@Component({
  selector: 'app-work-apporder-unhandleall',
  templateUrl: './work-apporder-unhandleall.component.html',
  styleUrls: ['./work-apporder-unhandleall.component.css']
})
export class WorkApporderUnhandleallComponent implements OnInit {
  ...
  workOrderUnHandledInfo(index = 0) {
    ...
    this.http.get(AppEnvConfig.env + "/work/workProdOrderUnHandleInfoAll?index=" + index, options)
        .map(res => res.json())
  ...
  infoTotalCount(self, cb) {
    ...
    self.http.get(AppEnvConfig.env + "/work/unHandleProdOrderInfoAllCount", options)
        .map(res => res.json())
        .subscribe(data => {
          if (data.status == 200) {
