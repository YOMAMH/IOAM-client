...
@Component({
  selector: 'app-al-eip',
  templateUrl: './al-eip.component.html',
  styleUrls: ['./al-eip.component.css']
})
export class AlEipComponent implements OnInit {
            ...
            if (data.status != 500) {
              ...
              self.addLocalInstanceInfoAll(function (data) {
                if (data.status == 200) {
                  ...
                  self.baseTool.refView("alUHost/aleip");
                } else {
                  $('.conver').hide();
  ...
  addLocalInstanceInfoAll(cb, self) {
    ...
    self.http.get(`${AppEnvConfig.env}/alCloud/eip/cloudInstanceInfoCopy`, options)
        .map(res => res.json())
        .subscribe(data => {
          if (data.status == 200) {
            if (cb) cb(data);
  ...
  localInstanceInfoTotalCount(self, cb) {
    let options = self.baseTool.Header();
    self.http.get(`${AppEnvConfig.env}/alCloud/eip/localInstanceInfoCount`, options)
        .map(res => res.json())
        .subscribe(data => {
          if (data.status == 200) {
  ...
  localInstanceInfo(index = 0) {
    ...
    this.http.get(`${AppEnvConfig.env }/alCloud/eip/localInstanceInfo?index=${index}`, options)
    ...
    $(function () {
      ...
      $('.btn.btn-default.checkHostBtn').on('click', function () {
        ...
        if (idStr && regionStr) {
          ...
          self.http.get(`${AppEnvConfig.env}/alCloud/eip/instanceInfoALiCloud/?AllocationId=${idStr}&RegionId=${regionStr}`
  ...
  deleteLocalInstance(self, instanceId) {
    let options = self.baseTool.Header();
    self.http.delete(`${AppEnvConfig.env}/alCloud/eip/localInstanceInfoDelete?AllocationId=${instanceId}`, options)
        .map(res => res.json())
        .subscribe(data => {
          if (data) {
            if (data.status == 500) {
  ...
  createLocalInstance(self) {
    ...
    let options = self.baseTool.Header();
    self.http.post(`${AppEnvConfig.env}/alCloud/eip/localInstanceCreate`, body, options)
        .map(res => res.json())
        .subscribe(data => {
  ...
  updateLocalInstance(self, hostInfo, cb) {
    let body = JSON.stringify({RegionId: hostInfo.reg, AllocationId: hostInfo.instanceId});
    let options = self.baseTool.Header();
    self.http.put(`${AppEnvConfig.env}/alCloud/eip/localInstanceUpdate`, body, options)
        .map(res => res.json())
        .subscribe(data => {
          cb();
          if (data) {
  ...
  searchLocalInstanceInfo(self) {
    $('.searchText').keydown(function (e) {
      if (e.keyCode == 13) {
        ...
        let options = self.baseTool.Header();
        if (instanceName) {
          self.http.get(`${AppEnvConfig.env}/alCloud/eip/localInstanceInfo?instanceName=${instanceName}`, options)
              .map(res => res.json())
              .subscribe(data => {
                $('.conver').hide();
                if (data.status == 200) {
          ...
          self.http.get(`${AppEnvConfig.env}/alCloud/eip/localInstanceInfo?index=0`, options)
              .map(res => res.json())
              .subscribe(data => {
                $('.conver').hide();
                if (data.status == 200) {
  ...
  selectBox(self) {
    $(function () {
      $('.hostBanner .hostSelectBox div').on('click', function () {
        ...
        if ($(this).find('span').text() == "公网弹性IP管理") {
          ...
          self.baseTool.getHistory(self.baseTool.resultView, self, "/alCloud/eip/localInstance?begDate=");
        } else {
  ...
  getLocalInstanceDropInfo() {
    ...
    this.http.get(`${AppEnvConfig.env}/alCloud/eip/localInstanceDropInfo`, options)
        .map(res => res.json())
        .subscribe(data => {
          if (data.status == 200) {
  ...
  cancelDropLocalInstancet(instanceId, cb) {
    ...
    this.http.put(`${AppEnvConfig.env}/alCloud/eip/cancelDropLocalInstance`, body, options)
        .map(res => res.json())
        .subscribe(data => {
          if (data.status == 200) {
            this.baseTool.alertView("还原成功！", '#38AF57');
            cb();
            this.baseTool.refView("alUHost/aleip");
          } else {
  ...
  localInstanceupdateAll(self) {
    $(function () {
      $('#updateAllBtn').on('click', function () {
        ...
        self.http.get(`${AppEnvConfig.env}/alCloud/eip/localInstanceUpdateAll`, options)
            .map(res => res.json())
            .subscribe(data => {
...
function handleView(data) {
  ...
  if (data) {
    data.forEach((item, i) => {
      ...
      if (item.InternetChargeType === "PayByBandwidth") {
        $('.hostTab .hostItem').eq(i).children().eq(2).children('span').eq(0).text("按固定带宽计费");

      }

      if (item.ChargeType === "PostPaid") $('.hostTab .hostItem').eq(i).children().eq(3).find('span').text("按量付费");

      if (item.Status === "InUse") {
        $('.hostTab .hostItem').eq(i).children().eq(4).find('span').text("已分配").css('color', '#2fd62f');
        $('.hostTab .hostItem').eq(i).children().eq(4).find('i').css('color', '#2fd62f').addClass("fa fa-circle");
      }

      let InstanceTypeStr = item.InstanceType.substring(0,item.InstanceType.indexOf('Instance')).toLocaleUpperCase();
      InstanceTypeStr = `${InstanceTypeStr} 实例`;
      $('.hostTab .hostItem').eq(i).children().eq(6).text(InstanceTypeStr);
...
function handleCloudData(data, self) {
  if (data.status == 200) {
    if (data.result.hasOwnProperty('content')) {
      ...
      let time = new Date(hostSet['AllocationTime']).toLocaleString();
      self.createLocalInstanceInfo = hostSet;
      $('.hostInfoContent').html('<ul>' +
          '<li><span>资源ID</span><span>' + hostSet['AllocationId'] + '</span></li>' +
          '<li><span>带宽</span><span>' + hostSet['Bandwidth'] + 'Mbps' + '</span></li>' +
