...
@Component({
  selector: 'app-apm-server',
  templateUrl: './apm-server.component.html',
  styleUrls: ['./apm-server.component.css']
})
export class ApmServerComponent {
  ...
  public actionUrl: string = "#/apm/server";
  ...
  constructor(private route: ActivatedRoute, private router: Router, private http: Http, private cdr: ChangeDetectorRef) {
    ...
    this.ApmBase.getHostInfo(this, "/apm/server");    // 获取产品线数据
    this.ApmBase.getOrderProdMaxRound(this,"tingyun_server");    // 应用变更频率top5
  }

  ngOnInit() {
    this.ApmBase.pagetion(this, "/apm/server");    // 分页
    this.ApmBase.handleCreateHost(this, "听云Server", "/apm/creatServer");    // 创建应用
    this.ApmBase.handleDeleteHost(this, "/apm/dropServer");    // 删除主应用
    this.ApmBase.searchHost(this, "/apm/server");    // 模糊搜索
    this.ApmBase.updateProducts(this, "/apm/updateServer");    // 更新数据
