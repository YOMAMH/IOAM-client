...
@Component({
  selector: 'app-apm-network',
  templateUrl: './apm-network.component.html',
  styleUrls: ['./apm-network.component.css']
})
export class ApmNetworkComponent {
  ...
  public actionUrl: string = "#/apm/network";
  ...
  constructor(private route: ActivatedRoute, private router: Router, private http: Http, private cdr: ChangeDetectorRef) {
    ...
    this.ApmBase.getHostInfo(this, "/apm/network");    // 获取产品线数据
    this.ApmBase.getOrderProdMaxRound(this,"tingyun_network");    // 应用变更频率top5
  }

  ngOnInit() {
    this.ApmBase.pagetion(this, "/apm/network");    // 分页
    this.ApmBase.handleCreateHost(this, "听云Network", "/apm/creatNetwork");    // 创建应用
    this.ApmBase.handleDeleteHost(this, "/apm/dropNetwork");    // 删除主应用
    this.ApmBase.searchHost(this, "/apm/network");    // 模糊搜索
    this.ApmBase.updateProducts(this, "/apm/updateNetwork");    // 更新数据
