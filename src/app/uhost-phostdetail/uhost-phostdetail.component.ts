...
@Component({
  selector: 'app-uhost-phostdetail',
  templateUrl: './uhost-phostdetail.component.html',
  styleUrls: ['./uhost-phostdetail.component.css']
})
export class UhostPhostdetailComponent implements OnInit {
  ...
  // 动画效果
  anmite() {
    $('.setInfo').addClass('setInfoAnmite').removeClass('setInfoInit');
    $('.baseInfo').addClass('baseInfoAnmite');
    $('.payInfo').addClass('payInfoAnmite');
  }

  // 初始位置
  setTransform() {
    $(function () {
      $('.setInfo').addClass('setInfoInit');    // Y轴旋转90度
      $('.baseInfo').addClass('baseInfoInit');    // 平移左侧
      $('.payInfo').addClass('payInfoInit');    // 平移右侧
    });
  }
  ...
  hostList(self) {
    $('.orderContainer .hostTab').delegate('.detailBtn', 'click', function () {
      ...
      let appName = $.trim($(this).parent().parent().children().eq(1).find("span").text());
