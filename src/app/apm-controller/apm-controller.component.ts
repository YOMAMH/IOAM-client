...
@Component({
  selector: 'app-apm-controller',
  templateUrl: './apm-controller.component.html',
  styleUrls: ['./apm-controller.component.css']
})
export class ApmControllerComponent implements OnInit {
  ...
  public actionUrl: string = "#/apm/controller";
  ...
  constructor(private route: ActivatedRoute, private router: Router, private http: Http, private cdr: ChangeDetectorRef) {
    ...
    this.ApmBase.getHostInfo(this, "/apm/controller");    // 获取产品线数据
    this.ApmBase.getOrderProdMaxRound(this,"tingyun_controller");    // 应用变更频率top5
  }

  ngOnInit() {
    this.ApmBase.pagetion(this, "/apm/controller");    // 分页
    this.ApmBase.handleCreateHost(this, "听云Controller", "/apm/creatController");    // 创建应用
    this.ApmBase.handleDeleteHost(this, "/apm/dropController");    // 删除主应用
    this.ApmBase.searchHost(this, "/apm/controller");    // 模糊搜索
    this.ApmBase.updateProducts(this, "/apm/updateController");    // 更新数据
